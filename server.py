...
def handle_client(client_socket, address, file_path):
    ...
    try:
        ...
        # Decode the received bytes to a string
        search_string = data.decode('utf-8').rstrip('\x00').strip()
        log_debug_info(f"IP: {address[0]}, Search Query: '{search_string}'")
        ...
        # Log execution time and search query
        log_debug_info(f"Execution Time: {elapsed_time:.2f} ms")
        ...
        log_debug_info(f"Connection closed with {address[0]}")
        # Close the connection
        client_socket.close()
        print(f"[INFO] Connection closed with {address}")
